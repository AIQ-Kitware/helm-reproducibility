...
Fifth follow-up during the same session: the user still saw `constrained_layout not applied because axes sizes collapsed to zero` on the per-metric agreement plot. That plot is a dense facet grid, and the global constrained-layout spacing knobs can make Matplotlib's layout solver give up even when the figure is fairly large. I changed only that plot to use manual `subplots_adjust` spacing with a slightly taller per-row figure size, while leaving the other plots on constrained layout. This is less magical for a dense grid and keeps the layout defaults useful without letting one plot family collapse the layout engine. After the user pointed me to `/home/joncrall/code/helm_audit/.venv313/bin/python`, I verified the real report redraw. I also made the manual-grid adapter translate `h_pad`/`w_pad` into subplot margins so all `PlotLayout` defaults have a visible effect there, and loosened the layout-default test so ongoing research tuning of the numeric defaults does not break the suite.

Validation: `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 /home/joncrall/code/helm_audit/.venv313/bin/python -m pytest -q tests/test_core_metrics_single_run.py tests/test_rebuild_core_report.py` passed with 17 tests. A compile check on the touched Python files also passed. I ran the exact real targeted redraw with `PYTHON_BIN=/home/joncrall/code/helm_audit/.venv313/bin/python` and confirmed it wrote a fresh `core_metric_per_metric_agreement` artifact without the constrained-layout warning.
