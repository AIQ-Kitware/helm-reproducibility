...
def legacy_repo_publication_root() -> Path:
    ...
    Used only by :mod:`helm_audit.workflows.analyze_experiment` to migrate
...
def portable_repo_root_lines(repo_root_fallback: Path | None = None) -> list[str]:
    ...
    ``helm_audit/``. If the script lives outside the repo (the new
    ...
    if repo_root_fallback is None:
        ...
        from helm_audit.infra.paths import repo_root as _repo_root
        repo_root_fallback = _repo_root()
    fallback_repr = str(repo_root_fallback).replace('"', '\\"')
    return [
        'SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"',
        'if [[ -n "${REPO_ROOT:-}" && -f "$REPO_ROOT/pyproject.toml" && -d "$REPO_ROOT/helm_audit" ]]; then',
        '  :',
        'else',
        ...
        '  done',
        'fi',
        'if [[ ! -f "$REPO_ROOT/pyproject.toml" || ! -d "$REPO_ROOT/helm_audit" ]]; then',
        '  echo "Could not locate helm_audit repo root from $SCRIPT_DIR (fallback ' f'{fallback_repr}' ' also missing); set REPO_ROOT=" >&2',
        '  exit 1',
        'fi',
